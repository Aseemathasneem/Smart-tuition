...
export default function StudentOtpVerification() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const response = await apiCall('post', endpoints.STUDENT_POST_OTP_VERIFICATION, { email, otp });
    ...
    const handleResendOtp = async () => {
        try {
            ...
            const data = await apiCall('post', endpoints.STUDENT_RESEND_OTP, { email });
            setLoading(false);
            ...
            if (data.success === false) {
                ...
                startCountdown(120); // Start countdown from 2 minutes (120 seconds)
            }
        } catch (error) {
            if (error.response && error.response.data) {
                setErrorMessage(error.response.data.message);
